...
| **退避机制数量** | 称"4 套" | 实际是 5 套（漏了 `BACKOFF_RESET_THRESHOLD`），在统一表里列齐 |
...
| 4 | **五套退避机制各自为政** | Guardian restart / HeartbeatEngine recovery / auth suppression / user message cooldown / backoff reset threshold | 见 §4.5 |
...
对外 `agent-status.json` 只写 `"health": "unavailable"`，下游 c4-receive / c4-dispatcher 无需区分两个子阶段。
...
```json
{
  ...
  "auth_retry_suppressed_until": null
}
```

PM2 重启时 `loadInitialHealth()` 读取这个文件恢复完整退避上下文，而不是只读 health 值。
...
| 2 | Guardian backoff reset | Guardian | 连续稳定 60s 后 restart counter 归零 | v1 遗漏的第 5 套 |
| 3 | HealthEngine recovery（Unavailable first-stage） | HealthEngine | `60s → 300s → 1500s → 3600s cap` | 内部探测间隔 |
| 4 | HealthEngine recovery（Unavailable degraded） | HealthEngine | 固定 3600s 间隔 | 60min 后降级 |
| 5 | Auth retry suppression | Guardian | 固定 180s | AuthFailed 后的 restart 抑制 |
| 6 | User message cooldown | HealthEngine | 60s | 防止用户消息触发重复探测 |
...
Socket 协议简单（5 个命令：`ROUTE` / `QUERY_HEALTH` / `ACK_HEARTBEAT` / `FORCE_CHECK` / `PING`），失败时 c4-receive 回退到"直接 tmux 投递 + 异步日志"的降级路径——这保证即使 MessageRouter 挂了，用户消息不会丢。
...
Hook 脚本只**写文件**，不调用主进程。主架构通过 SignalStore 统一消费。这个契约不变，v2 只是澄清了哪个 hook 写哪个文件——`hook-activity.js` → `tool-events.jsonl`（不是 `api-activity.json`）。
...
| 消费者 | 改动 | 归属 Phase |
|--------|------|----------|
| `c4-receive.js:224`（`health === 'down'` 分支） | 改为 `health === 'unavailable'` | Phase 5 |
| `c4-dispatcher.js:584`（`health !== 'ok'`） | 无需改动 | — |
| `web-console/scripts/server.js` | 无需改动 | — |
| UI health 显示 | 两个子阶段仍然合并显示为 "unavailable" | Phase 5（文案） |
...
5. **（v2 新增）MessageRouter socket 失败策略？**
   **推荐：降级到直接 tmux 投递 + 告警**。socket 不可用时保证消息不丢，同时 log 一条 ERROR 供观察。不进入 Unavailable（不是健康问题，是通信问题）。
