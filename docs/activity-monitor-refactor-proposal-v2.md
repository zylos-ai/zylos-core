...
| 分类 | v1 的问题 | v2 的修正 |
|------|-----------|----------|
| **架构盲区** | 完全未纳入 PR #500 的 watchdog/tool-lifecycle 子系统（934 LoC） | 新增第四条主循环步骤 `toolTracker.tick()`，watchdog 独立为一等公民模块 |
...
- §4.8：MessageRouter socket 失败时**不做** direct-to-tmux，走 C4 主链降级；新增"C4 主链原则"
...
**群内 ccb981c2 确认后的二次修订（2026-04-17 晚）**：
- §4.8.1 新增：pin 住 happy path 的 DELIVERED 语义——MessageRouter **不调 tmux**，所有 tmux 写入仍由 c4-dispatcher 独占；并发聚合只作用于 recovery check 阶段，OK 路径不聚合。这条补丁把 v1 "direct-to-tmux happy path" 的隐含歧义彻底清除，与 §4.8 的"C4 主链原则"形成闭环
- §4.8.2（原 §4.8 fallback 段落重命名，无实质改动）
...
| 5 | **Watchdog 子系统游离于主架构** | 934 LoC（tool-lifecycle 442 + tool-event-stream 241 + tool-watchdog 145 + session-foreground 55 + claude-pid 51；main@a738d0a 实测）在主循环中深度集成但无清晰边界 | PR #500 新增 |
...
#### 4.8.1 DELIVERED 语义与 tmux 投递边界（happy path pin-down）

即便 socket 正常工作，MessageRouter **本身不调 tmux**——所有 tmux 写入仍由 `c4-dispatcher.js` 独占负责，保留 DB priority sort、`require_idle` 门控、`health === 'ok'` 门控的全部现有约束（`c4-dispatcher.js:572-660`）。

Socket 协议中 `ROUTE` 返回值的准确语义：

> **DELIVERED** ≡ "消息已进入 C4 主链正常投递流程（`insertConversation` 已在 `c4-receive.js:261` 完成），health 为 OK，dispatcher 会按 priority 正常处理"。**不代表任何 tmux 写入动作**，也不代表 dispatcher 已经 ack。

配套返回值与 c4-receive 行为：

| ROUTE 返回 | 含义 | c4-receive 对用户的回复 |
|-----------|------|------------------------|
| `DELIVERED` | health=OK，走正常主链 | "消息已收到" |
| `STATUS recovering` | Unavailable 前期，已触发 recovery check | "agent 恢复中，请稍候" |
| `STATUS degraded` | Unavailable 后期 | "agent 暂不可用" |
| `STATUS auth_failed` | AuthFailed | "需要重新登录" |
| `STATUS rate_limited` | RateLimited | "限流中，约 X 分钟后恢复" |

并发聚合（`#pendingCheck` + `#waitingMessages`，见下方代码）**只作用于 recovery check 阶段**——在 Unavailable 状态下让多条同时到达的消息共享一次探测，避免 N 次重复检查。OK 路径下不做聚合：每条消息独立写 DB 返回 DELIVERED，投递顺序由 DB priority + dispatcher 保证，不经过 MessageRouter。

这个边界确保：

1. Socket 不破坏 DB 的 priority 序列（dispatcher 全链路保留）
2. "direct-to-tmux" 从 happy path 也彻底消失（v2 review 已拆 fallback 路径，本节补齐 happy path 的显式约束）
3. MessageRouter 的职责限定在**健康协调层**（回答"当前健康吗" + 并发聚合 recovery 探测），不是**投递层**
4. Multi-channel 并发（Telegram / Lark / web-console / shell / HXA 五路同时到达）不产生 tmux send-keys 字符交叉——tmux 写入仍然由 dispatcher 单线独占

#### 4.8.2 socket 失败的降级路径（v2 review 后修正）

c4-receive **不做** direct-to-tmux 投递——那条路径会绕开 C4 的 DB audit、priority ordering、`require_idle` 门控、`agentState.health === 'ok'` 门控（`c4-dispatcher.js:572-660`），违反 comm-bridge 的核心约束。
...
这类 hook 通过 `c4-control.js` 向 C4 控制队列入队操作建议，仍然经过 C4 主链（**不违反 §4.8 的 C4 主链原则**）——DB audit / priority / require_idle 照走，只是入口是 hook 而不是外部消息。
...
5. **（v2 新增，review 后修正）MessageRouter socket 失败策略？**
   **推荐：消息入 C4 队列 + 跳过 MessageRouter 的主动聚合，不做 direct-to-tmux**。socket 不可用时让 c4-dispatcher 按现有 health gating 继续投递，保证 DB audit / priority / require_idle 都保留（见 §4.8 "C4 主链原则" + §4.8.1 DELIVERED 语义边界）。同时 log 一条 ERROR 供观察。不进入 Unavailable（不是健康问题，是通信问题）。
