...
> v1 留存于 `activity-monitor-refactor-proposal.md` 作为历史参照；本方案为 v2 详细设计档（**已 SUPERSEDED**）。
...
- **Activity State（活动状态）**：Offline / Idle / Busy — 描述进程是否在运行
...
6. **SignalStore 只读快照、Adapter 依赖注入、TaskScheduler 注册式调度、Hook 路径不变**：四个架构基石。Hook 脚本文件路径不动，用户 `settings.json` 无需修改。
...
**合并 Offline / Stopped 的理由**：二者对消费端完全同义（都是"需要拉起"），差异只在 Guardian 拉起路径的实现（要不要先建 tmux session）。把实现细节下沉到 Guardian 内部，不让它泄漏到对外 ActivityState 契约。消费端的 state 值域从 4 收敛为 3。

**冻结的瞬态处理**：ProcSampler 检测到进程冻结（context switch 长期为零）后直接 kill，**不在 agent-status.json 写 frozen 状态**。下一 tick Guardian 看到进程消失 → 写 Offline → 拉起流程接管。冻结是实现层的瞬态，不进入对外 ActivityState 枚举（否则消费端要处理额外状态，且会让"实现细节泄漏到契约"的反模式出现）。冻结事件走日志层（`activity.log` + metrics），不污染状态契约。合并 Offline/Stopped 遵循同一条原则——"Guardian 内部的拉起路径差异"也是实现瞬态，不进契约。
...
| 参数 | 归属 | 默认 | 作用 |
|------|------|------|------|
| `startupGrace` | Guardian 层 | 30s | Guardian 的"本地锁"——防止 tmux session 尚未建立就触发第二次 `startAgent()` |
| `launchGracePeriod` | HealthEngine 层 | 180s | 抑制 HealthEngine 的主动探测，避免新进程初始化期间误判为 Unavailable |

两个参数的作用域完全不同：一个控制 Guardian 什么时候允许判断"进程不存在需要拉起"，另一个控制 HealthEngine 什么时候开始主动探测。合并会导致 Guardian 等 180s 才敢判断进程不存在——30s 已足够且必要。两者均暴露为 per-runtime 配置项（Claude 可调 60s / 120s，Codex 保持 30s / 180s）。
...
PR #500 引入的 tool-lifecycle / tool-event-stream / tool-watchdog / session-foreground / claude-pid 子系统在本方案中作为**一等公民**接入主循环编排。
...
bypass-once 覆盖**默认** cold start 路径。当操作员手动修好根因（改了 API key、升级了依赖、清理了坏缓存）并希望 AM 立即从 `initial_delay` 重试时，通过一个**显式 opt-in 机制**传达 operator intent：
...
```
on AM cold start:
    if exists(~/zylos/activity-monitor/.reset-request):
        guardian.state.reset()         # 清空 4 字段到默认值
        unlink marker                  # one-shot，用完即焚
        bypassBackoffOnce = false      # 已经是干净起点，不需要 bypass
        log "operator-triggered reset"
    else:
        guardian.state.loadFromFile()  # 从 guardian-state.json 恢复（默认）
        bypassBackoffOnce = true       # bypass-once 覆盖首次尝试（默认）
```

**操作员 CLI**：

```
zylos am reset-backoff            # 只 touch marker，等下次 restart 生效
zylos am reset-backoff --restart  # touch marker + pm2 restart activity-monitor（一条命令）
```

**三种 AM 冷启动场景的语义矩阵**：

| 场景 | marker 存在？ | Guardian 4 字段 | 首次探测 | 语义 |
|------|--------------|----------------|---------|------|
| 操作员 `zylos am reset-backoff --restart` | ✅（一次性）| **清零** | 从 `initial_delay` 开始 | operator 显式意图，立即重试 |
| daily-upgrade 完成后 AM 自重启 | ❌ | 保留持久化 | bypass-once 给 1 次机会 | 若 upgrade 修好，1 probe 成功后清空；否则回持久化水位 |
| PM2 auto-restart（AM 崩溃恢复）| ❌ | 保留持久化 | bypass-once 给 1 次机会 | 故障认知跨重启保留，不被 AM 自身稳定性影响 |

**为什么不靠"所有 cold start 都清零"的简单方案**：那样无法区分"operator 意图再试"和"AM 自己崩了 10 次 PM2 在救"两种场景，后者每次从 `initial_delay` 重爬会对 runtime / auth / 外部 API 产生持续脉冲。用 marker 做显式 opt-in，operator intent 和 auto-restart 的行为正交，两边都能取到合适语义。

**marker 的 one-shot 语义很重要**：清完必须删掉。否则每次 cold start 都 reset，等于"所有 cold start 清零"的退化行为。实现上**在 reset 完成后立即 unlink**，不等 exit（防 crash 丢失清理）。
...
**HealthEngine 不对外暴露同步查询方法**（不存在 `isRestartBlocked()` / `getBackoffDelay()` 这类 getter）。限流冷却状态写入 SignalStore 的 `rate-limit-state.json`（进入 RateLimited 时写 `{until: <timestamp>}`，退出时清空），Guardian 从 SignalStore 消费。Guardian 和 HealthEngine 之间只存在"事件类"单向调用（Guardian → HealthEngine 的 `setAuthFailed` / `onProcessRestarted`；ToolWatchdog / MessageRouter → HealthEngine 的 `triggerRecovery` / `notifyUserMessage`），**反向查询路径不存在**。

**传播延迟契约（API contract）**：Guardian 经 SignalStore 消费 HealthEngine / TaskScheduler 的信号，**一致性模型是 eventual consistency**。生产者在 tick 中写入后，下一个 tick 开头 SignalStore.refresh() 读回，同一 tick 内消费者观察到——**上界 = 1 tick（≈1s）**，`rate-limit-state.json` 和 `maintenance-state.json` 均遵循此契约。Guardian 的拉起决策显式接受这个滞后：极端场景下限流解除瞬间 Guardian 最多慢 1s 才拉起新进程，代价可控；收益是 Guardian 与 HealthEngine / TaskScheduler 的同步调用路径完全消除。任何未来新增的"Guardian 经 SignalStore 消费的信号"必须遵守同样的 ≤ 1 tick 契约，不引入更长延迟通道。
...
c4-receive 在 health 非 OK 时**同步等 in-flight probe 结果**（短 window + timeout fallback），再给用户**唯一一次真实回复**。这条语义落地了 §1.2 原则 1 的"基于真实 probe 的答复"要求，保证"一次 c4-receive 一次真实答案"不变量——任何把 liveness 判断下放给"等更久看 AI 是否回"的隐式轮询（如先回 interim ACK 再靠 AI 后续回复的双消息模式）都会打破这条不变量，不采用。
...
`insertConversation` 只在 OK 直投或 probe recovered 分支里执行。STATUS 回复分支**不写 DB**——消息在这一次交互中完结，不留给 dispatcher 再次处理。因为 STATUS 回复的消息从未进入 DB，"STATUS 回复后消息是否会被 dispatcher 再处理"的答案是：不会。用户若要重试，STATUS 文案明示"请恢复后重发"。这是"一次 c4-receive 一次真实答案"不变量的物理保障。
...
读 `agent-status.json` + `rate-limit-state.json`（§3.3），不直接调 HealthEngine 方法。符合 §1.2 原则 1 "状态共享走 SignalStore"——MessageRouter 不引用 HealthEngine 字段，所有 health 状态都通过 SignalStore 通道消费。

**C4. IPC 不可用时的降级**

c4-receive 连不上 MessageRouter（monitor.js crash / socket down）时：
- **不 `insertConversation`**——消息不进 DB 队列，dispatcher 后续不会处理它
- 回一条 terminal 文案：`"router 暂时不可用，请稍后重发"`
- 用户凭文案决定是否重发；MessageRouter 恢复后再来的消息走正常路径

**"一次 c4-receive 一次真实答案" 不变量在所有路径下 100% 成立**——降级路径不再有"2 条回复"的例外。代价是 IPC 降级期间新消息不进队列，operator 必须靠用户重发；这是可接受的，因为 MessageRouter 不可用属于 monitor.js crash 级别的异常，正常路径下不出现。文案明确告知用户"请稍后重发"，不做假入队承诺。
...
- **OK / probe recovered 路径不丢**：`insertConversation` 保证消息入队后最终由 dispatcher 处理
- **IPC 降级路径不入队**：不做假入队承诺；terminal 文案要求用户重发（C4）
- **并发聚合**：MessageRouter 在 recovery check 阶段共享 in-flight probe（§3.6）
- **dispatcher 约束保留**：health gating + priority + `require_idle` 全链路约束不变
- **不提供"立即反馈"的 interim ACK**：任何"消息已入队"的回复都不等于"系统能处理"，会把一条 receive 变成两条 bot reply，违反 "一次 c4-receive 一次真实答案" 不变量。用户 Grace 期间等 10-30s 看到真实答复，依据原则 "宁可慢，不要假"。

### 3.8 ToolPipeline（物理合并 PR #500 tool-lifecycle + tool-event-stream）

PR #500 引入了一套完整的工具生命周期子系统。本方案将其中相关的两个文件**物理合并**为 **`tool-pipeline.js`**（不是仅概念合并），作为主循环的一等公民：
...
当前散落的多套退避在本方案中有明确的归属与参数，合并视图如下：
...
"5 + 1" 的说法：5 套主动退避 + 1 个重置阈值。本方案合并在同一张表里，便于审计与调参。
...
PR #500 的 watchdog 子系统已在 main 上运行两个月，**内部状态机 / 规则数值 / intervention 按键序列完全保留**。但边界（文件组织、接口面、依赖注入点）按本方案架构适配——避免出现"下游 c4-receive 按新架构改、Watchdog 却豁免"的原则 4 矛盾。
...
1. 在 monitor.js 进程内实例化 MessageRouter，暴露本地 IPC 入口
2. c4-receive 改造：按 §3.7 流程**同步等 MessageRouter 响应**（OK 直投；Unavailable/AuthFailed/RateLimited 聚合 recovery check 后按结果分支）；不 `exit(1)`；STATUS 文案路径**不 `insertConversation`**
3. MessageRouter 实现 recovery check 阶段的并发聚合（OK 路径不聚合）
4. c4-dispatcher 适配新 health 值域
5. 测试：并发聚合测试（多消息共享一次探测）、IPC 降级测试（IPC 不可用时 c4-receive 返回 terminal 文案、消息不入队）、硬超时 30s fallback 测试、§3.7 C1~C4 全链路不变量测试
