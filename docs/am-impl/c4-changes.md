...
```javascript
// IPC server handler（当前由 activity-monitor.js 通过 Orchestrator 启动）
...
`content` 必须以 `Heartbeat check.` 开头并包含 `[phase=...]`。runtime heartbeat control 规则以该前缀匹配 heartbeat，并解析 `phase`；`phase='recovery' | 'post_restart'` 表示 HealthEngine recovery probe。

recovery heartbeat 不得传 `--no-ack-suffix`。C4 必须附加当前 control id 的 `ack via: node ... c4-control.js ack --id <id>` 后缀，runtime heartbeat control 规则通过执行该 ack 命令完成显式 ack。这里的“禁用 auto-ack”只禁止 dispatcher 自行 mark done，不是删除 ack suffix。
...
**Recovery heartbeat 禁用 auto-ack**。`shouldAutoAckHeartbeat()` 不得对 `phase='recovery' | 'post_restart'` 返回 true。这两类 heartbeat 只有 runtime heartbeat control 规则识别 `Heartbeat check. [phase=...]` 并执行 ack suffix 后才能变为 `done`，因为它们用于验证 runtime liveness。
...
3. **c4-dispatcher.js** — 新增 `notifyMessageDelivered()`；在 `markDelivered()` 后异步调用；`import net`；确保 recovery heartbeat control bypass health gate、优先投递，`phase='recovery' | 'post_restart'` 不走 auto-ack，且仍保留 ack suffix 供 runtime heartbeat control 规则显式 ack
4. **AM 侧 IPC handler** — handler 新增 `notify_delivered` 分支，调用 `healthEngine.onUserMessageDelivered()`；当前主入口是 `monitor.js`，`activity-monitor.js` 保持兼容薄入口
5. **测试** — c4-receive IPC 正常/fallback/超时；c4-receive unhealthy 路径 exit code；c4-dispatcher 通知成功/AM 未运行；noReply 静默；concurrent route + notify 不冲突；health=unavailable 时 recovery heartbeat 仍会投递；recovery heartbeat content 为 `Heartbeat check. [phase=recovery|post_restart]` 且带 ack suffix；recovery heartbeat 不会被 auto-ack；runtime heartbeat control 规则 ack 后 heartbeat 才变 done；runtime 不响应时 25s 后 HealthEngine 返回 `heartbeat_timeout`
