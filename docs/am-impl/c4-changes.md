...
`content` 必须以 `Heartbeat check.` 开头并包含 `[phase=...]`。runtime hook 以该前缀匹配 heartbeat，并解析 `phase`；`phase='recovery' | 'post_restart'` 表示 HealthEngine recovery probe。

recovery heartbeat 不得传 `--no-ack-suffix`。C4 必须附加当前 control id 的 `ack via: node ... c4-control.js ack --id <id>` 后缀，runtime hook 通过执行该 ack 命令完成显式 ack。这里的“禁用 auto-ack”只禁止 dispatcher 自行 mark done，不是删除 ack suffix。
...
**Recovery heartbeat 禁用 auto-ack**。`shouldAutoAckHeartbeat()` 不得对 `phase='recovery' | 'post_restart'` 返回 true。这两类 heartbeat 只有 runtime hook 识别 `Heartbeat check. [phase=...]` 并执行 ack suffix 后才能变为 `done`，因为它们用于验证 runtime liveness。
