...
### 3.1 模块职责表（11 项：10 业务模块 + 1 Adapter DI）
...
| **HealthEngine** | `scripts/health-engine.js` | 健康状态机 + 主动探针编排 + api-error catalog dispatch | 6 |
...
> **InputValidator** 是**可选增强**，**不属于 v2.1 implementation baseline**——详见 §5.5。如启用，加在 tick 第 2 步（其他步骤数顺移）；不启用时本表 8 步 tick 是核心。
...
> **如启用 §5.5 InputValidator 可选增强**：在 ① 之后插入 `inputValidator.tick()` 作为 step 2，其他步骤数顺移成 9 步 tick。**MVP baseline 不启用**——上面是 8 步主流程。
...
| `validate-request/<msgid>.json` | c4-receive（带 attachment 消息到达时写）| InputValidator（tick 处理）— **仅 §5.5 启用时存在** |
| `validate-result/<msgid>.json` | InputValidator（tick 处理完写，one-shot）| c4-receive（轮询读，处理后 unlink）— **仅 §5.5 启用时存在** |
| ~~`pending-channels.jsonl`~~ | **从目标方案删除**（main 既有异步恢复广播路径在 v2.1 废弃，详 §5.3.2 + §九 迁移项）| n/a |
...
tick(signals):
  ...
  ⑤ probe 结果回调更新 state
  ⑥ 若 state 变动：更新内存 + 写/清 rate-limit-state.json（**不再 drain pending-channels**——Direction D 下用户问询和健康检测已经同步，unhealthy 路径已即时返回 outbound 状态文案，无需异步恢复广播；详 §5.3.2）
```
...
main 已有 `pending-channels.jsonl` 机制（c4-receive 在 health 非 OK 时记录被拒收的 channel/user，AM 恢复后异步通用 broadcast"我恢复了"）—— **v2.1 在 Direction D 下废弃这条异步恢复广播路径**。本节不引入新的受害者识别 ledger，且**砍掉 main 旧的异步恢复广播**（理由：用户问询和健康检测同步——unhealthy 路径已即时返回 outbound 状态文案，无需事后再发通用 "我恢复了"，否则会造成双通知/语义冗余），并**显式声明数据完整性 + 用户体验的 boundary 边界**，以及 session 恢复后的对话续接机制。
...
main 既有的 pending-channels.jsonl 异步恢复 broadcast 路径**在 Direction D 下废弃** ——unhealthy 路径已经在 c4-receive 同步返回 outbound 状态文案给用户，AM 无需再维护一个延迟"我恢复了"广播列表（双通知 + 语义冗余）。迁移期 c4-receive 旧的"非 OK 时写 pending-channels.jsonl"分支删除；AM 旧的"recovery 时 drain"职责删除。
...
4. **不**主动 broadcast"我恢复了"——unhealthy 时用户已经同步收到 outbound 状态文案，无需再异步通知
...
### 5.5 InputValidator — ⚠️ 可选增强 / NOT IN MVP BASELINE

> **范围说明 (R1 review P3 #4 收敛)**：
> - **本节不属于 v2.1 implementation baseline**——MVP 不实施 InputValidator
> - §3.1 模块表 / §3.3 主循环 tick 不含 InputValidator（**8 步 tick** 为核心；启用本附录时插入第 2 步成 9 步）
> - §九 Phase 1 不含 `input-validator.js`
> - 即使完全不实施 InputValidator，§5.3.1 catalog 出口治理 + §5.3.2 c4-session-init context 注入已经**完整覆盖正确性边界**
> - 本附录的价值是**减少已知规格违规走 60-70s 出口治理的 latency**——把"图大 / 格式错"等 90% 已知 case 在 1.5s 内入口拦下提升 user 体验。**纯 latency / UX 优化层**
> - 未来如要纳入 baseline 需另开 PR + 升级测试为必选

如启用，tick 第 2 步插入。**runtime-specific 输入校验**，配合 §5.3.1 的出口治理形成**双层防御**——前线拦截已知规格违规，兜底处理意外漏过的。
...
- **API error catalog（§5.3.1）**：`getApiErrorPatterns()` 返回 catalog 数组，每 entry 含 `{id, pattern, severity, recoveryAction, debounce, scanInterval, userMessage}`。HealthEngine 读取后在 Layer 2 tmux scan 中按 `recoveryAction` dispatch（**adapter 不得同时支持 `action` alias，避免旧草稿字段漂移**）
...
| ~~**InputValidator + Adapter `getInputRules`/`validateInput`**~~ | — | — | — | — | — | — |
| ↑ **NOT IN MVP BASELINE**（详 §5.5 可选增强；如未来纳入需另开 PR + 升级测试为必选）| | | | | | |
...
**注**：早期 v2.1 草案曾设计 `recent-inbound.jsonl` + `pending-channels` 升级 + `restart-in-progress.json` barrier + `recent-inbound.lock` + `lastSafeIdleTs` activity-driven cleanup + cold-restart broadcast 整套受害者识别链路，经多轮 review walkback 后**全部砍掉**——理由详见 §5.3.2。Direction D 下：c4 DB（`insertConv` inbound + outbound 双写）+ `c4-session-init.js` (unsummarized context 注入) 已经覆盖正确性边界；main 旧的 `pending-channels.jsonl` 异步恢复广播路径同步**废弃**（v2.1 迁移期 c4-receive 旧分支删除 + AM 旧 drain 职责删除）。
...
新建 `signal-store.js` / `status-writer.js` / `task-scheduler.js` / `tool-pipeline.js` + `tasks/` 目录（6 任务独立文件 + 1 stale-signal 清理任务）。旧 `activity-monitor.js` 仍在，新模块通过 feature flag 挂接，独立可 ship。

> **不含 `input-validator.js`** 也不含 Adapter `getInputRules()` / `validateInput()` 接口扩展——这两块都是 §5.5 可选增强，不在 MVP baseline。
