...
| `in-flight/<msgid>.json` | c4-receive（insertConversation 后写）/ c4-dispatcher（submit-verify 成功后 unlink） | HealthEngine（session_restart 时扫描衍生 pending-channels）/ stale-signal GC |
| `pending-channels.jsonl` | HealthEngine（**session_restart 时扫 in-flight 目录衍生**，附 unavailable_reason + userMessage）| AM 恢复后主动 broadcast 通知 |
...
- 升级后流程同 `session_restart`：`adapter.stop()` + 进 Unavailable + 走 §5.3.2 in-flight signal 衍生 pending-channels；broadcast 通用文案「⚠️ session 因未识别的运行时错误已重置，请重发；具体原因已记录待人工诊断」
...
v2.1 增强为**带诊断的恢复通知**——**严格限定在 catalog action = `session_restart` 时才触发**（其他 action 走 §5.3.1.1 的自然恢复路径，不进本机制）。**affected channel 来源从 in-flight 目录扫描衍生**，覆盖 A/B/C 三类用户。

##### In-flight signal 机制（受害者识别）

```
[c4-receive]
   ① InputValidator + MessageRouter 通过后，insertConversation 成功
   ② 立即写：in-flight/<msgid>.json = { channelId, userId, msgId, ts }

[c4-dispatcher]
   ① 取 DB 消息 → tmux submit
   ② submit-verify 通过（确认进了 Claude session） → unlink in-flight/<msgid>.json
   ③ submit 失败重试机制不变；in-flight 不被 unlink → 下一次 session_restart 仍能识别该受害者

[HealthEngine session_restart 触发时]
   ① 扫描 in-flight/ 全部未 unlink 文件 = 飞行中受害消息集合
   ② 每条衍生 pending-channels.jsonl entry：
        { channelId, userId, msgId, ts, unavailable_reason: '<catalog.id>',
          userMessage: '<catalog.userMessage>' }
   ③ unlink 已记录的 in-flight 文件（防 restart loop 同一 msgId 再次通知）
...
- **C1 Migration ordering（先迁 AM 后迁 dispatcher）**：Phase 2 部署 AM in-flight scan，Phase 3 同步部署 dispatcher unlink。反序会让 in-flight 文件永不 unlink → 假阳性 broadcast 堆积；不反序问题不大（多余 unlink 无害）
- **C2 写 pending-channels 必须先于 stop**：扫 in-flight + 写完 pending-channels.jsonl 后才调 `adapter.stop()`。反序会让 c4-dispatcher 在 stop 边缘继续 unlink 而丢失受害者；正序保证 stop 那一刻 in-flight 集合完整捕获
- **C3 受害者去重（restart loop 防呆）**：写完 pending-channels 立即 unlink in-flight；同一 msgId 不会跨 restart 重复 broadcast。用户硬要重发同图：会写**新的** in-flight/<新 msgId>，下轮仍会通知（msgId 不同）——这是预期行为（每次重发都该被告知失败），但每分钟 ≤1 次受 60-70s cycle 自然限速
- **C4 Fail-open**：写 in-flight 失败（磁盘满 / fs 异常）→ c4-receive 不阻塞主链，仅 log warning 继续 insertConversation；该消息成为沉默受害者（回退 main 行为，不更糟）。扫 in-flight 失败 → 仍 stop（保安全），broadcast 退化为通用文案"session 已重置，刚发的消息可能丢失，请检查后重发"。**故障下退化到通用通知 + 自然恢复，不阻塞 session_restart 主路径**

##### Broadcast 文案（诚实版）

catalog entry 的 `userMessage` 给精确诊断；HealthEngine 拼接通用前后缀后发给用户：

```
"⚠️ 我刚才处理你发的消息时遇到了运行时问题——

[catalog.userMessage]

session 已重置（之前的对话上下文已丢失）：
- 如果是同一内容触发的（如同张图），**再发会再次失败**，请修改后重试
- 如果想问刚才发生了什么，直接发文本问我即可"
```
...
- channel-aware：仅通知 in-flight 真实受害者；其他 channel 第三方完全不打扰
- 通知触发条件可配置（per-channel 默认 on，可关）

**为什么不"unknown 也广播"**：unknown 默认 action 是 `unavailable_with_probe` 不重启 session，用户消息**还在 DB 队列**等 probe 通过——主动 broadcast"已重置"会误导。**例外**：§5.3.1 的"持续未匹配升级"路径——5min 后被升级为 session_restart，此时 in-flight 受害者也走本机制 broadcast 通用文案"未识别错误，session 已重置"。

##### 用户视角的 UX 改善

| 错误类型 | catalog action | 当前 main 用户体验 | v2.1 (E+) 用户体验 |
|---------|---------------|------------------|-------------------|
| 已知 sticky（图片 400 / context 超长） | `session_restart` | 60s 静默 + 无解释 + 原消息丢 | 60-70s 后**精确诊断 broadcast** + 文案诚实告知再发会失败 |
| 同期 B 类（pre-detection 发的）| 跟随触发者 | 跟 A 一样 silent | 跟 A 一样收到 broadcast（in-flight 含 B）|
...
**核心改进**（v2.1 E+ vs main）：(1) 场景 2/4（A 触发者 + 同期 B 类）从"完全静默" → "收到精确诊断 broadcast"——in-flight signal 让 broadcast 知道通知谁；(2) 场景 5（unknown sticky 长期）从"永久卡死" → "5min 自愈 + 通用通知"——unknown 持续性升级修复 v2.1 fallback 自困死的退步；(3) 不引 cooldown / blacklist → 文本对话不被误伤。
...
MessageRouter 读 health（SignalStore 投射）
    ├─ OK              → c4-receive 写 DB **+ 写 `in-flight/<msgid>.json`** → dispatcher 主链（submit-verify 后 unlink in-flight）→ AI 真实回复
    ├─ Unavailable     ↴
...
| **In-flight signal 机制（受害者识别：c4-receive 写 + dispatcher unlink + AM 扫描衍生）** | | | **✅ AM 端** | **✅ dispatcher 端** | | |
...
- **In-flight signal AM 端（§5.3.2）**：HealthEngine session_restart 触发时扫 `in-flight/` 目录衍生 pending-channels；先扫后 stop 顺序保证（C2）；写完 pending-channels 后 unlink in-flight（C3）；fail-open（C4）
...
- c4-receive 改造：
  ...
  - **insertConversation 成功后写 `in-flight/<msgid>.json`**（受害者识别信号，§5.3.2）
- c4-dispatcher：**submit-verify 通过后 unlink `in-flight/<msgid>.json`**（先迁 AM 后迁 dispatcher，§5.3.2 C1）；适配新 health 值域
- 测试：并发聚合 / IPC 降级（terminal 文案 + 不入队）/ 硬超时 30s / C1~C4 不变量 / **InputValidator 拦截路径 / fail-open 路径 / 与 catalog A+ 配合的双层防御 E2E** / **in-flight signal 受害者识别（A 触发者 + B 类同期 + 写 pending-channels 顺序约束 C2 / 去重 C3 / fail-open C4）/ unknown 5min 升级路径 E2E**
