...
- ❌ InputValidator 入口校验（原 §5.5 整章 + Adapter `getInputRules()` / `validateInput()` 接口）—— zylos 当前架构下**用户消息附件走 path-as-text**：channel daemon 把图片下载本地后只把**文件路径作为字符串**塞进 `c4-receive --content`，conversations 表只有 TEXT 列，**不存在自动 multimodal 注入路径**。坏图永远不会自动进 Claude API user turn，agent (Claude Code) 看到的是文本里的路径，**由 agent 自己决定调 Read 工具加载**。Read 工具自带 size / format 边界 → 超限直接返 error 给 agent → agent 在对话里告诉用户重发，**不会触发 sticky API error → catalog → restart 链**。原 problem statement（拦下 90% 已知规格违规避免 60-70s 出口治理 latency）的前提不成立——因为根本不存在"坏图被自动送进 API → sticky 4xx"的链路。未来若新增 multimodal 直接注入路径（base64 content block 等），正确的入口校验位置是该注入点（runtime-aware by design），不是独立 AM module。
...
| 新 `monitor.js` 编排 | | | ✅ 新建 | | | |
| MessageRouter | | | | ✅ 新建 | | |
| c4-receive 适配（同步等 MessageRouter + unhealthy 路径写 outbound 状态文案）| | | | ✅ | | |
| c4-dispatcher 值域 | | | | ✅ | | |
