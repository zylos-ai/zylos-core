...
| `recent-inbound.jsonl` | c4-receive（每条入站消息 append，**持 `recent-inbound.lock` 写入**）| HealthEngine（session_restart 时 snapshot + 升级为 pending-channels，**同锁下操作**）/ stale-signal GC（**活动驱动清理**：trim `ts < lastSafeIdleTs` 条目；**+ 1hr wall-clock fail-safe**：trim `ts < NOW-3600s` 防 Claude 卡死场景；**同锁下操作**） |
| `recent-inbound.lock` | c4-receive / HealthEngine / stale-GC （沿用 `cli/lib/lock.js` O_EXCL + PID + ts stale 检测模式；**正交两个 timeout**：`acquireTimeoutMs=1s` (c4-receive 等锁 patience) + `staleTimeoutMs=60s` (前任 owner 卡死兜底)）| 同上三方互斥访问 `recent-inbound.jsonl` |
...
#### `lastSafeIdleTs`（活动驱动 turn-safe 锚点，§5.3.2 受害者识别清理用）

HealthEngine 维护一个内存字段 `lastSafeIdleTs` —— **最近一次 Claude turn 完成且 HealthState 全程 OK 的时间戳**。

**更新条件**（必须三条同时满足）：
1. ActivityState 转 `Idle`（来自 ToolPipeline `Stop` hook 触发的 turn 完成信号）
2. 本次 `Busy → Idle` 期间 HealthState 一直保持 `OK`（无 catalog hit / 无 setAuthFailed / 无 RateLimited）
3. 不在 `launchGracePeriod` 内（避免冷启动初始 Idle 假触发）

满足时：`lastSafeIdleTs = Date.now()`。否则**不更新**——保留旧值。

**用途**（详见 §5.3.2 stale-GC）：recent-inbound.jsonl 中 `ts < lastSafeIdleTs` 的条目可安全清除（这些消息已被一轮 clean turn 处理过）。

**为什么不用 wall-clock TTL**：Zylos 框架长 turn 场景（Memory Sync / OpenClaw daily / `/ultrareview` / 长任务）经常 > 5min，wall-clock TTL 会过早清除还在处理中的 recent-inbound 条目，导致 sticky 时漏报 silent loss。`lastSafeIdleTs` 是 turn-完成驱动的真实安全锚点。

**Wall-clock fail-safe**：在 `lastSafeIdleTs` 之外仍设 `recent-inbound` entry 1 小时绝对兜底——极罕见 Claude 卡死永远不 Idle 时（PM2 级 incident）防文件无限膨胀；触发即 silent loss 接受作为兜底代价。
...
| `notify_only` | 仅 log + 可选 user 即时通知（**不写 pending-channels，不进 cold-restart broadcast**——避免误导用户以为消息丢失），不改 health | 轻量级警告（content filter 等可恢复非 sticky 类）|
...
- 用户文案 fallback：「服务暂时不可用，正在自动重试探测——请稍候，如长时间无响应请重发」（**注意**：unknown fallback 默认 action 是 `unavailable_with_probe` 不重启，所以文案不能说"已重置"）

**持续未匹配升级（防卡死兜底）**：
- 同一 unknown error pattern **连续 10 次扫描命中**（30s × 10 = 5min）→ **升级到 `session_restart` action**
...
- 升级后流程同 `session_restart`：`adapter.stop()` + 进 Unavailable + 走 §5.3.2 recent-inbound 升级机制生成 pending-channels；broadcast 通用文案「⚠️ session 因未识别的运行时错误已重置，请重发；具体原因已记录待人工诊断」
...
**渐进可知**：catalog 不是一次性写死，是**活的知识库**。今天的 unknown 通过 jsonl 累积 + review，明天就是 known + 精确文案；未升级的 known 路径走精确诊断 broadcast，升级路径走通用文案 + 后续补 catalog。
...
main 已有 `pending-channels.jsonl` 机制（broadcast"我恢复了"），但**不附原因**且**来源不准**——只记录 health 已切非 ok 后到达的 C 类用户，对真正受害者（A 触发者 + 同期 B 类）一无所知。

v2.1 增强为**带诊断的恢复通知**——**严格限定在 catalog action = `session_restart` 时才触发**（其他 action 走 §5.3.1.1 的自然恢复路径，不进本机制）。**affected channel 来源从 `recent-inbound.jsonl`（活动驱动滚动活动日志）snapshot 升级而来**，覆盖 A/B/C 三类用户。

##### Recent-inbound 活动日志 + pending-channels 升级流程

设计选择：**不引入新的 per-msg 信号目录或 turn-risk 抽象**，而是在 c4-receive 维护 `recent-inbound.jsonl` 活动日志；**清理触发器是 `lastSafeIdleTs`（活动驱动）而非 wall-clock TTL**——只在 Claude 完成一轮 clean turn 且 HealthState 全程 OK 后才清理对应窗口前的条目。这避免了 Zylos 长 turn 场景（Memory Sync / `/ultrareview` / 长任务等 > 5min 路径）下漏清未处理消息的问题。HealthEngine 在 session_restart 时把当前未清理的 entry 整体升级为 pending-channels broadcast 目标。
...
[c4-receive 收消息后]
   ...
   ② read SignalStore.health (cached snapshot)
        ...
        └─ OK → 继续
   ③ acquireLock('recent-inbound', acquireTimeoutMs=1s, try/finally)
   ...
   ⑤ 【lock 内】insertConversation + append recent-inbound.jsonl：
        ...
        // 不写消息 content；活动驱动 cleanup（typically 一个 turn 内）+ 1hr fail-safe 限制 PII 在制时长
   ⑥ releaseLock（finally 块保证）

[stale-signal GC tick 任务（同锁，活动驱动）]
   ① acquireLock('recent-inbound', acquireTimeoutMs=1s, try/finally)
   ② trim recent-inbound.jsonl 中满足以下任一条件的条目（tmp+rename atomic）：
        - ts < lastSafeIdleTs（活动驱动主路径：Claude 已完成一轮 clean turn 处理这些消息）
        - ts < NOW - 3600s（wall-clock fail-safe：极罕见 Claude 卡死永远不 Idle 时兜底）
   ③ releaseLock
...
[HealthEngine 触发 session_restart action 时]
   ...
   ③ 【lock 内】snapshot recent-inbound.jsonl
       filter: ts > lastSafeIdleTs AND ts < barrier_ts
              （自然只剩"自上次 clean Idle 以来 + barrier 之前"的飞行中条目；
               stale-GC 已清理过 ts < lastSafeIdleTs 的，filter 是自防御冗余）
...
[notify_only / unavailable_with_probe / rate_limit_cooldown action]
   不进入本流程；recent-inbound 自然在下次 clean Busy→Idle 时被 GC 清掉（活动驱动），无副作用
```
...
锁特性（**两个 timeout 必须正交**，不能合一）：
- **`acquireTimeoutMs=1s`（client patience）**：c4-receive 等锁多久放弃。1s 是高频 intake 路径的合理上限，超过则走 C4 lock fail-open 退化无锁 append + log warn
- **`staleTimeoutMs=60s`（stale safety net）**：前任 owner 持锁超过此时长才判定卡死并强删。**必须远长于任何合法操作时长**——HealthEngine snapshot+clear 实际 10-50ms，60s 是 1000-6000× 富余；触发即说明 owner 真卡死（PM2 级 incident）
- **stale 检测优先级**：① `isProcessRunning(pid) === false` 立即强删（owner 已死，instant pickup）→ ② `lockAge > staleTimeoutMs` 兜底强删（owner alive 但卡死的极罕见场景）
...
**为什么必须正交两个 timeout**：如果合一为单一"timeout=1s"，HealthEngine snapshot+clear 偶尔慢点（fs cache miss / 大文件 / GC pause / 1.2s）会被 c4-receive acquire 时误判 stale 强删 → c4-receive 拿锁 append → HealthEngine 还在 clear → **Layer A 锁直接失效，append vs rewrite race 重新打开**。这是 zylos0t round-4 review 抓到的实施契约 bug。

**lock.js 复用建议**（实施层面，非本 PR doc-only）：把 `cli/lib/lock.js` 重构为 `lib/lock.js` 通用 helper，API 改为 `acquireLock(name, { acquireTimeoutMs, staleTimeoutMs })`；CLI 用 `acquireTimeoutMs=∞ + staleTimeoutMs=10min`，AM 用 `acquireTimeoutMs=1s + staleTimeoutMs=60s`。一次提升、双方受益。
...
2. acquireLock('recent-inbound', { acquireTimeoutMs: 1000, staleTimeoutMs: 60000 })
...
3. acquireLock('recent-inbound', { acquireTimeoutMs: 1000, staleTimeoutMs: 60000 })
...
| HealthEngine 持锁过久阻塞 intake | Layer A `acquireTimeoutMs=1s` + try/finally release 限制 c4-receive worst case；C4 fail-open 兜底 |
| c4-receive crash 持锁 | Layer A stale 检测：`isProcessRunning(pid) === false` 立即强删 |
| HealthEngine 慢操作（snapshot+clear 偶尔 1-2s） | Layer A `staleTimeoutMs=60s` 远长于操作时长，**不会**被误判 stale 强删（避免 race 重新打开）|
| HealthEngine 真卡死（极罕见 PM2 级 incident） | Layer A `staleTimeoutMs=60s` 兜底强删，让其他 owner 能 pickup（接受小概率 race 换 liveness）|
| Lock 模块自身故障 | C4 fail-open：c4-receive 退化无锁 append；HealthEngine 退化强 stop + 通用文案；stale-GC 跳过本轮 |

##### 为什么不让 dispatcher 参与

之前的 in-flight signal 设计让 c4-dispatcher 在 `submit-verify` 后 unlink，被 zylos0t 指出**`submit-verify` 不是 turn-safe 证明**——它只验证 `tmux send-keys Enter` 后输入框清空，即"paste 进了 Claude 输入框"；坏图 sticky API error 在此之后 1-15s 才 surface（见 c4-dispatcher.js `submitAndVerify()` 实际语义）。

新设计**完全绕开 dispatcher**：c4-receive 是入站消息的 ground truth 时间点，TTL 兜底覆盖整个 turn 周期，不依赖任何"成功" signal。dispatcher 维持 §5.4 既有职责（DB 取消息 → tmux submit），不参与 victim 识别。

##### Broadcast 文案（诚实版）

catalog entry 的 `userMessage` 给精确诊断；HealthEngine 拼接通用前后缀后发给用户。文案使用 generic 措辞（不指定具体几分钟），因为 `lastSafeIdleTs` 活动驱动 cleanup 后剩余窗口跟实际 turn 时长动态匹配，而非固定 wall-clock：
...
session 已重置（正在处理中的对话上下文已丢失）：
- 如果你刚才发的某条消息没收到回复，请重发
- 如果是同一内容触发的（如同张图），**再发会再次失败**，请修改后重试
- 如果 Claude 已经回复过，那条不受影响"
```

**要点**：
- **不加 cooldown / hash blacklist 防呆**——靠 60-70s restart cycle 自然限速 + 诚实文案让用户主动判断
- 通知**走 C4 主链**（c4-control enqueue），保留 audit trail，priority 最高
- channel-scoped：活动窗口内 (channel, endpoint) 维度 de-dup，每对话最多收一条；窗口外 / 其他 channel 第三方不打扰
- **假阳性容忍**：bystander（活动窗口内发了无关消息且已收到回复）理论上也可能被通知，但**活动驱动 cleanup 大幅减少这类 case**——已被 Claude 完整回复过的对话在下次 clean Idle 后即清理；只有"Claude 处理多条 message 中途 sticky"才会让前面已回复的 bystander 也进 broadcast。文案诚实告知"如果 Claude 已经回复过那条不受影响"——可忽略
- 通知触发条件可配置（per-channel 默认 on，可关）

**为什么不"unknown 也广播"**：unknown 默认 action 是 `unavailable_with_probe` 不重启 session，用户消息**还在 DB 队列**等 probe 通过——主动 broadcast"已重置"会误导。**例外**：§5.3.1 的"持续未匹配升级"路径——5min 后被升级为 session_restart，此时 recent-inbound 窗口内的活动也走本机制 broadcast 通用文案"未识别错误，session 已重置"。

##### 用户视角的 UX 改善

| 场景 | catalog action | 当前 main 用户体验 | v2.1 用户体验 |
|---------|---------------|------------------|-------------------|
| 已知 sticky 触发者 A（图片 400 / context 超长） | `session_restart` | 60s 静默 + 无解释 + 原消息丢 | 60-70s 后**精确诊断 broadcast** + 文案诚实告知再发会失败 |
| 同期 B 类（pre-detection 发的）| 跟随触发者 | 跟 A 一样 silent | 跟 A 一样收到 broadcast（recent-inbound 活动窗口含 B）|
| 活动窗口内 bystander（已收到 Claude 回复的）| 跟随触发者 | 不通知 | 大多数情况**不通知**——已回复的对话在下次 clean Idle 后被活动驱动 GC 清掉。**仅"Claude 处理多条 message 中途 sticky"边缘 case 才误通知**（文案引导自判断，轻度噪音）|
| Unknown 4xx 短期（catalog 未匹配，5min 内） | `unavailable_with_probe` (fallback) | 60s 静默 → unknown 不被处理 | dispatcher probe 通过 → **消息自然投递**（用户没察觉异常）；同时 jsonl 累积 |
| Unknown 4xx 长期（5min 持续） | **升级 → `session_restart`** | **永久卡死**（probe 永远失败长退避）| **5min 自愈 + 通用 broadcast** + 后续人工补 catalog |
| 503 / overload | `unavailable_with_probe` | 60s 静默 → 自然投递 | 同（消息延迟投递，无 broadcast）|
| 限流 | `rate_limit_cooldown` | 现有 cooldown 通知 | 同 + dispatcher 自然恢复 |
| C 类（health 已非 ok 后到达）| 任何 action | 现有 pending-channels 通用 broadcast | 同上 + 文案带 unavailable_reason |

**核心改进**（v2.1 vs main）：
1. **A 触发者 + 同期 B 类从"完全静默" → "收到精确诊断 broadcast"** — recent-inbound 活动窗口让 HealthEngine 知道通知谁
2. **Unknown sticky 长期从"永久卡死" → "5min 自愈 + 通用通知"** — §5.3.1 持续未匹配升级机制修复 v2.1 fallback 在 sticky context 下自困死
3. **不引 cooldown / blacklist / per-msg signal 文件** — 文本对话不被误伤；活动驱动 cleanup（`lastSafeIdleTs` + 1hr fail-safe）简化 lifecycle，无 magic 抽象、无 wall-clock 上限假设
4. **代价**：边缘 case 下 bystander 收到一条可忽略的通知（文案引导自判断），换来 silent loss 彻底消除——活动驱动 cleanup 大幅减少这类 case 比 wall-clock TTL
...
MessageRouter 读 health（SignalStore 投射）
    ├─ OK              → c4-receive 写 DB **+ append `recent-inbound.jsonl`** → dispatcher 主链 → AI 真实回复
    ├─ Unavailable     ↴
...
- 同一清理任务上挂另一条规则：**`recent-inbound.jsonl` 每秒活动驱动 trim**——清除满足 `(ts < lastSafeIdleTs OR ts < NOW - 3600s)` 的条目（见 §5.3.2 / §5.3 lastSafeIdleTs；活动驱动主路径 + 1hr wall-clock fail-safe）。**操作前必须 `acquireLock('recent-inbound', { acquireTimeoutMs: 1s, staleTimeoutMs: 60s })`，try/finally release**；rewrite 走 `tmp + rename` atomic 模式。lock 故障时跳过本轮（C4 fail-open）
...
| **Recent-inbound 活动日志（受害者识别）+ pending-channels 升级** | | | **✅ AM 端 snapshot/升级** | **✅ c4-receive append** | | |
| **Unknown error 持续性升级（5min → session_restart）** | | | **✅** | | | |
| 新 `monitor.js` 编排 | | | ✅ 新建 | | | |
| MessageRouter | | | | ✅ 新建 | | |
...
- **Recent-inbound 升级 AM 端（§5.3.2，含并发安全合约 + 活动驱动 cleanup）**：HealthEngine session_restart 触发时 ① publish `restart-in-progress.json` barrier → ② acquireLock(`recent-inbound`, `acquireTimeoutMs=1s`) → ③ snapshot recent-inbound（**filter ts > lastSafeIdleTs AND ts < barrier_ts** + de-dup by channel+endpoint）→ ④ 生成 pending-channels entry → ⑤ 清空已升级 → ⑥ releaseLock → ⑦ adapter.stop() → ⑧ OK 转换后 drain pending-channels broadcast + unlink barrier。HealthEngine 加 `lastSafeIdleTs` 内存字段：仅在 `Busy → Idle` 转换 AND 本次 Busy 期间 HealthState 一直 OK 时更新（避免 wall-clock TTL 过早清未处理消息，应对 Zylos 长 turn 场景）。Lock 沿用 `cli/lib/lock.js` O_EXCL + PID + ts stale 模式（建议重构为通用 `lib/lock.js` API `acquireLock(name, { acquireTimeoutMs, staleTimeoutMs })`，**两个 timeout 必须正交**：CLI 用 `staleTimeoutMs=10min`，AM 用 `acquireTimeoutMs=1s + staleTimeoutMs=60s`）；try/finally release。stale-signal 清理任务挂活动驱动 trim 规则（`ts < lastSafeIdleTs OR ts < NOW-3600s`）同样持锁（§5.5）。Layer A 锁解 Race 1（append vs rewrite），Layer B barrier 解 Race 2（snapshot 后 intake leak）；C1（barrier+lock 顺序）/ C2（去重）/ C3（功能 fail-open）/ C4（lock fail-open）
...
- c4-receive 改造：
  ...
  - **OK 路径下 acquireLock(`recent-inbound`, `{acquireTimeoutMs:1s, staleTimeoutMs:60s}`) → re-check `restart-in-progress.json` barrier → insertConversation + append `recent-inbound.jsonl` → releaseLock**（try/finally；受害者识别信号，§5.3.2 并发安全合约 Layer A + B）
- c4-dispatcher：适配新 health 值域（**不参与 recent-inbound 生命周期**——§5.3.2 设计去耦）
- 测试：并发聚合 / IPC 降级（terminal 文案 + 不入队）/ 硬超时 30s / C1~C4 不变量 / **InputValidator 拦截路径 / fail-open 路径 / 与 catalog 出口治理配合的双层防御 E2E** / **recent-inbound 受害者识别（A 触发者 + B 类同期 / barrier+lock 顺序 C1 / 升级后清空防 loop C2 / 功能 fail-open C3 / lock fail-open C4 / 假阳性 group bystander 文案引导）/ probe-recovered 也 append recent-inbound / unknown 5min 升级路径 E2E** / **活动驱动 cleanup（§5.3 lastSafeIdleTs）：clean Busy→Idle 转换更新 lastSafeIdleTs / sticky 期间不更新 / Zylos 长 turn (Memory Sync / ultrareview / 30min 任务) 场景下不漏清未处理 / Claude 卡死 1hr fail-safe 触发兜底 / 多 message 队列 lastSafeIdleTs 不误更新** / **并发安全合约（§5.3.2）：Layer A 锁——多 c4-receive 并发 append+stale-GC trim+HealthEngine snapshot/clear 互斥不丢写；Layer B barrier——HealthEngine snapshot 后 c4-receive 在 lock 内 re-check barrier 拦下 intake leak；stale-lock recovery（PID dead 立即强删 / lockAge > staleTimeoutMs=60s 兜底强删）；c4-receive crash try/finally 不持锁；HealthEngine 慢操作 1-2s 不会被 acquireTimeoutMs=1s 误判 stale（staleTimeoutMs=60s 远长于操作时长）；c4-receive acquireTimeoutMs=1s 后 fail-open 退化无锁 append**
