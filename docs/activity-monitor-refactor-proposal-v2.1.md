...
1. **双层正交状态机**：ActivityState 与 HealthState 相互独立运行，跨模块交互通过**具名接口**（而非读对方字段）进行，例如 `engine.getBackoffDelay()` / `engine.setAuthFailed()` / `engine.triggerRecovery()`。**具名接口是正交原则的实现方式，不是对正交的削弱**——接口调用不等于耦合。每个接口都有明确且收敛的语义，不是通用的 `getHealth` / `setHealth`。
...
第一条是 ActivityState 读 HealthState 的唯一场景，通过具名接口封装，遵循 §1.2 正交原则——接口调用不是耦合。

Guardian 在 auth-check 失败时调用 `engine.setAuthFailed(reason)` 通知 HealthEngine——这是一个收敛的具名接口，不是通用 `setHealth`，符合 §1.2 正交原则。
...
- `onProcessRestarted()` — Guardian 调用：从 `agent-status.json` 读取持久化 health 回填内存状态，启动 launchGracePeriod 抑制探测，重置退避计时，clear pending；**不强制修改 health**（两层正交，进程拉起 ≠ 健康变化）
- `setAuthFailed(reason)` — Guardian 调用：将状态切到 AuthFailed
- `triggerRecovery()` — ToolWatchdog / MessageRouter 调用：主动发起一次恢复检查（门控规则见下）
...
这是 §1.2 具名接口交互的应用：通过明确收敛的接口名暴露，不扩展为通用写接口。
