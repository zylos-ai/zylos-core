...
v2.1 增强为**带诊断的恢复通知**——**严格限定在 catalog action = `session_restart` 时才触发**（其他 action 走 §5.3.1.1 的自然恢复路径，不进本机制）。**affected channel 来源从 `recent-inbound.jsonl`（5min 活动日志）snapshot 升级而来**，覆盖 A/B/C 三类用户。

##### Recent-inbound 活动日志 + pending-channels 升级流程

设计选择：**不引入新的 per-msg 信号目录或 turn-risk 抽象**，而是在 c4-receive 维护一个 5min 滚动活动日志 `recent-inbound.jsonl`；HealthEngine 在 session_restart 时把窗口内的 entry 整体升级为 pending-channels broadcast 目标。
...
catalog entry 的 `userMessage` 给精确诊断；HealthEngine 拼接通用前后缀后发给用户。文案明示**最近 5min 窗口**（因为 recent-inbound 是窗口语义，不是精确单消息）：
...
- channel-scoped：5min 窗口内 (channel, endpoint) 维度 de-dup，每对话最多收一条；窗口外 / 其他 channel 第三方不打扰
- **假阳性容忍**：bystander（5min 内发了无关消息且已收到回复）也会被通知。文案诚实告知"如果 Claude 已经回复过那条不受影响"——可忽略
...
| 同期 B 类（pre-detection 发的）| 跟随触发者 | 跟 A 一样 silent | 跟 A 一样收到 broadcast（recent-inbound 5min 窗口含 B）|
| 5min 内 bystander（已收到 Claude 回复的）| 跟随触发者 | 不通知 | ⚠️ **会收到通知** — 文案"如果 Claude 已经回复过那条不受影响"引导其忽略；轻度噪音 |
...
**核心改进**（v2.1 vs main）：
1. **A 触发者 + 同期 B 类从"完全静默" → "收到精确诊断 broadcast"** — recent-inbound 5min 窗口让 HealthEngine 知道通知谁
2. **Unknown sticky 长期从"永久卡死" → "5min 自愈 + 通用通知"** — §5.3.1 持续未匹配升级机制修复 v2.1 fallback 在 sticky context 下自困死
3. **不引 cooldown / blacklist / per-msg signal 文件** — 文本对话不被误伤；TTL 5min 兜底简化 lifecycle，无 magic 抽象
4. **代价**：5min 内 bystander 收到一条可忽略的通知（文案引导自判断），换来 silent loss 彻底消除
...
- 同一清理任务上挂另一条规则：**`recent-inbound.jsonl` 每秒 trim ts > 5min ago 的条目**（见 §5.3.2 受害者识别机制）。**操作前必须 `acquireLock('recent-inbound', { acquireTimeoutMs: 1s, staleTimeoutMs: 60s })`，try/finally release**；rewrite 走 `tmp + rename` atomic 模式。lock 故障时跳过本轮（C4 fail-open）
