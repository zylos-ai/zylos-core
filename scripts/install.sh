...
if ! _has_yes_flag && [ -t 0 -o -e /dev/tty ]; then
  ...
  echo "  │                                                        │"
  printf '%b' "${NC}"
  printf "  ${DIM}│${NC}  ${YELLOW}⚠ It is dangerous if untrusted people can${NC}          ${DIM}│${NC}\n"
  printf "  ${DIM}│${NC}  ${YELLOW}reach this machine or talk to the bot —${NC}             ${DIM}│${NC}\n"
  printf "  ${DIM}│${NC}  ${YELLOW}they can execute anything.${NC}                          ${DIM}│${NC}\n"
  printf '%b' "${DIM}"
  echo "  │                                                        │"
  echo "  └────────────────────────────────────────────────────────┘"
  printf '%b' "${NC}"
