...
8. **Context Monitoring**: Receives context usage data after every turn; triggers early memory sync and new-session handoff at configurable thresholds (Claude runtime fallback 70%, Codex default 75%; fresh Claude installs and upgrade-time `opus[1m]` model backfills write explicit Claude threshold 30)
...
- **Two thresholds**:
  - **Early memory sync** at 80% of threshold (e.g. 56% for Claude runtime fallback 70%, or 24% when config explicitly sets 30): Enqueues a prompt for Claude to run memory sync as a background task, so it completes well before the session switch. Only triggers when unsummarized conversation count exceeds the checkpoint threshold (30).
  - **New-session handoff** at threshold (Claude runtime fallback `70%`, Codex default `75%`, configurable via `new_session_threshold` / `codex_new_session_threshold` in config.json): Enqueues the new-session trigger. Fresh Claude installs and upgrade-time model backfills to `opus[1m]` write explicit `new_session_threshold = 30` when the key is missing; existing configured values are preserved. Priority 1, 5-minute cooldown.
